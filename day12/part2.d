...
void main()
{
    ...
    foreach(y; 0 .. map.length)
    {
        foreach(x; 0 .. map[y].length)
        {
            ...
            if(map.cellAt(current) == 'S' || map.cellAt(current) == 'a')
            {
                start ~= current;
            }
    ...
    start.map!(s => map.findMinimalDistance(s, end)).filter!(length => length > 0).minElement().writeln();
}
...
unittest
{
    ...
    ulong result = start
        .map!(s => map.findMinimalDistance(s, Point(5, 2)))
